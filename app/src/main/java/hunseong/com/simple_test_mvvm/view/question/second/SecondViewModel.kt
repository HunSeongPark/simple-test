    ...
    fun calculatePercent(buttonType: ButtonType): Job = viewModelScope.launch {
        ...
        when (buttonType) {
            ButtonType.DEPRESSED -> {
                ...
                preferenceManager.setString(AppPreferenceManager.KEY_SECOND_ANSWER, "depressed")
            }
            ButtonType.SAD -> {
                preferenceManager.setInt(KEY_SAD_PERCENT, sadPercent + 5)
                preferenceManager.setInt(KEY_EXCITED_PERCENT, excitedPercent - 5)
                preferenceManager.setString(AppPreferenceManager.KEY_SECOND_ANSWER, "sad")
            }
            ButtonType.GOOD -> {
                preferenceManager.setInt(KEY_SAD_PERCENT, sadPercent - 5)
                preferenceManager.setInt(KEY_EXCITED_PERCENT, excitedPercent + 5)
                preferenceManager.setString(AppPreferenceManager.KEY_SECOND_ANSWER, "good")
            }
            ButtonType.EXCITED -> {
                preferenceManager.setInt(KEY_SAD_PERCENT, sadPercent - 10)
                preferenceManager.setInt(KEY_EXCITED_PERCENT, excitedPercent + 10)
                preferenceManager.setString(AppPreferenceManager.KEY_SECOND_ANSWER, "excited")
            }
        }
